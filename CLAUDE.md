...
Always start the dev server and test changes in a browser before reporting work as done. The drone page is at `/drone/` and the metronome at `/metronome/`.
...
Multi-page Vite app with vanilla TypeScript, no frameworks. Three entry points: landing (`index.html`), metronome (`metronome/index.html`), drone (`drone/index.html`). Shared CSS theme, no shared runtime code.
